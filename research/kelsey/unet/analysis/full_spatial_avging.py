"""
Scratch script to average runs for spatial plotting
"""
...
def spatial_map(uq_method, model_type, data, region, doy, savedir):
    """
...
if __name__ == '__main__':
    ...
    fns = os.listdir(dl_dir)
    arr_list = []
    for d in date_list:
        fns_doy = [x for x in fns if d in x]
        avg_list = []
        for f in fns_doy:
            np_arr = np.load('{}/{}'.format(dl_dir, f))
            avg_list.append(np_arr)
        arr_list.append(np.mean(avg_list, axis=0))

    for i in range(len(date_list)):
        #rf_data = rf_ds.sel(time=date_list[i])['0.9']
        spatial_map('DL UQ_avg', model, arr_list[i], geo, date_list[i], dl_dir)
        #spatial_map('RF_0.9_Quantile', model, rf_data, geo, date_list[i], dl_dir)
