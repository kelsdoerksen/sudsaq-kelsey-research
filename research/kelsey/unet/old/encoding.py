    ...
    def __init__(
        ...
        for _ in range(num_encoding_blocks):
            encoding_block = EncodingBlock(
                ...
                dilation=self.dilation,         # controls the spacing between the kernel points
                dropout=dropout
            )
...
class EncodingBlock(nn.Module):
    ...
    # PyTorch forward function to pass data through NN model -> x refers to the data
    def forward(self, x):
        if self.residual:
            connection = self.conv_residual(x)
            # Passing x through conv1
            x = self.conv1(x)
            # Passing x through conv2
            x = self.conv2(x)
            x += connection
        else:
            # Passing x through conv1
