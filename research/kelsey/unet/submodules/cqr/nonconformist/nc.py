...
def mask_nans(predictions, labels):
    ...
    preds_low = predictions[:, 0, :, :]
    preds_high = predictions[:, 0, :, :]
    ...
    preds_combo = preds_combo.reshape((len(labels_mask), 2))

    return preds_combo, labels_mask
...
class QuantileRegErrFunc(RegressionErrFunc):
    ...
    def apply(self, prediction, y):
        # Need to add again masking for nans
        pred_no_nan, y_no_nan = mask_nans(prediction, y)
        y_lower = pred_no_nan[:,0]
        y_upper = pred_no_nan[:,-1]
        error_low = y_lower - y_no_nan
        error_high = y_no_nan - y_upper
...
class RegressorNc(BaseModelNc):
    ...
    def predict(self, x, y, nc, significance=None):
        ...
        y: label locations of TOAR, used to filter out predictions
            we don't care about
        ...
        n_test = x.shape[0]
        prediction = self.model.predict(x)
        # Masking nans because we only apply calibration to predictions we care about
        preds_nonans, labels_nonans = mask_nans(prediction, y)
        if self.normalizer is not None:
            norm = self.normalizer.score(x) + self.beta
        else:
            norm = np.ones(n_test)
        ...
        if significance:
            intervals = np.zeros((x.shape[0], 2, preds_nonans.shape[0]))
            # TO UPDATE BELOW
