...
class IcpRegressor(BaseIcp, RegressorMixin):
    ...
    def predict(self, x, y, significance=None):
        """Predict the output values for a set of input patterns.

        Parameters
        ----------
        x : numpy array of shape [n_samples, n_features]
            Inputs of patterns for which to predict output values.
        ...
        if n_significance > 1:
            ...
            prediction = np.zeros((x.shape[0], 2, x.shape[2], x.shape[3]))
        ...
        for condition in self.categories:
            ...
            if np.sum(idx) > 0:
                p = self.nc_function.predict(x[idx, :], y,
